...
import { createMemory, getMemory, updateMemory } from '../services/MemoryService';
...
function MemoryComponent() {
  ...
  function saveMemory(e) {
    ...
    if (validateForm()) {
      const memory = { manufacturer, name, bought_price, selling_price, quantity_in_stock };
      console.log(memory);

      if (id) {
        updateMemory(id, memory).then((response) => {
          console.log(response.data);
          navigate('/memoryDevices');
        }).catch(error => {
          console.error(error);
        });
      } else {
        createMemory(memory).then((response) => {
          console.log(response.data);
          navigate('/memoryDevices');
        }).catch(error => {
          console.error(error);
        ...
        <div className='card col-md-6 offset-md-3 offset-md-3'>
          ...
          <div className='card-body'>
            <form>
              ...
              <button className='btn btn-success' onClick={saveMemory}>Підтвердити</button>
              <br/><br/>
