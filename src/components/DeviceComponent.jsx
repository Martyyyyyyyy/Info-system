...
const DeviceComponent = () => {
  ...
  function saveOrUpdateDevice(e) {
    e.preventDefault();

    if (validateForm()) {
      const device = { manufacturer, name, bought_price, selling_price, quantity_in_stock };
      console.log(device);

      if (id) {
        updateDevice(id, device).then((response) => {
          console.log(response.data);
          navigate('/inputdevices');
        }).catch(error => {
          console.error(error);
        });
      } else {
        ...
        <div className='card col-md-6 offset-md-3 offset-md-3'>
          ...
          <div className='card-body'>
            <form>
              ...
              <button className='btn btn-success' onClick={saveOrUpdateDevice}>Підтвердити</button>
              <br/><br/>
