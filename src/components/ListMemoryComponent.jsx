...
const ListMemoryComponent = () => {
   ...
   function addNewMemory(){
      navigator("/add-memory")
   }

   function updateMemory(id){
      navigator(`/edit-memory/${id}`)
   }

   function removeMemory(id){
      console.log(id);

      deleteMemory(id).then((response) =>{
  ...
  return (
    <div className='container'>
      ...
      <button className='btn btn-primary mb-2' onClick={addNewMemory}>Додати товар</button>
         ...
         <tbody>
            {
               memories.map(memory =>
                  <tr key={memory.id}>
                      ...
                      <td>{memory.name}</td>
                      <td>{memory.bought_price}</td>
                      <td>{memory.selling_price}</td>
                      <td>{memory.quantity_in_stock}</td>
                      <td>
                        <button className='btn btn-info' 
                        onClick={() => updateMemory(memory.id)}>Оновити товар</button>
