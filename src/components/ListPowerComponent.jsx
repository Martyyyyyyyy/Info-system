...
const ListPowerComponent = () => {
   ...
   function addNewPower(){
      navigator("/add-powerUnit")
   }

   function updatePower(id){
      navigator(`/edit-powerUnit/${id}`)
   }
  ...
  return (
    <div className='container'>
      ...
      <button className='btn btn-primary mb-2' onClick={addNewPower}>Додати товар</button>
         ...
         <tbody>
            {
               powers.map(power =>
                  <tr key={power.id}>
                      ...
                      <td>{power.name}</td>
                      <td>{power.bought_price}</td>
                      <td>{power.selling_price}</td>
                      <td>{power.quantity_in_stock}</td>
                      <td>
                        <button className='btn btn-info' 
                        onClick={() => updatePower(power.id)}>Оновити товар</button>
